...
export function useConnectMPIframe(iframe) {
	...
	return new Promise((resolve, reject) => {
		window.MP_SDK.connect(iframe, SDK_KEY, "")
			.then((sdk) => {
				resolve(sdk);
			})
			.catch((err) => {
 ...
 * @parmas {mpSdk} Object
 * @parmas {sid} string
 * @parmas {loadingImg} string
 *
 */
export async function useMatterTagLoading(mpSdk, sid, loadingImg) {
	let media = loadingImg ? { type: "mattertag.media.photo", src: loadingImg } : { type: "mattertag.media.none", src: "" };
	await mpSdk.Mattertag.editBillboard(sid, { media });
	return Promise.resolve();
}

/**
 * Get a matterTag by sid
 * @parmas {mpSdk} Object
 * @parmas {sid} string
 *
 */
export async function useGetMatterTag(mpSdk, sid) {
	let tags = await mpSdk.Mattertag.getData();
	let tag = tags.find((t) => t.sid === sid);
	return tag ? Promise.resolve(tag) : Promise.reject(null);
}

export async function useGetMatterTagPos(mpSdk, sid, $iframe, pose) {
	if (!(pose && pose.position)) return;
	const showcaseSize = {
		w: $iframe.clientWidth,
		h: $iframe.clientHeight,
	};
	var mattertag = await useGetMatterTag(mpSdk, sid);
	var screenCoordinate = mpSdk.Conversion.worldToScreen(mattertag.anchorPosition, pose.position, showcaseSize);
	console.log(screenCoordinate);
}

export async function useWatchCameraPos(mpSdk, cb) {
